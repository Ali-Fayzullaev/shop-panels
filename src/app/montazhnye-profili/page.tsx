import Header from "@/components/Header";
import Image from "next/image";
import { Button } from "@/components/ui/button";

export default function ProfilesPage() {
  return (
    <div className="min-h-screen bg-white">
      <Header />
      <main className="pt-6">
        <div className="container mx-auto px-4 py-8">
          <nav className="text-sm text-gray-500 mb-8">
            <a href="/" className="hover:text-gray-700">Главная</a>
            <span className="mx-2">/</span>
            <a href="/catalog" className="hover:text-gray-700">Каталог</a>
            <span className="mx-2">/</span>
            <span className="text-gray-900">Монтажные профили</span>
          </nav>

          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="relative aspect-4/3 overflow-hidden bg-gray-100">
              <Image
                src="/images/wall5.jpg"
                alt="Монтажные профили"
                fill
                className="object-cover"
              />
            </div>

            <div>
              <h1 className="text-4xl font-bold text-gray-900 mb-6">
                Монтажные профили
              </h1>
              
              <div className="prose prose-lg text-gray-700 mb-8">
                <p>
                  Качественные монтажные системы и профили для профессиональной 
                  установки стеновых панелей. Надежность и простота монтажа.
                </p>
                
                <h3 className="text-xl font-bold text-gray-900 mt-6 mb-4">Преимущества:</h3>
                <ul className="list-disc pl-6 space-y-2">
                  <li>Высококачественные материалы</li>
                  <li>Простота и скорость монтажа</li>
                  <li>Надежное крепление</li>
                  <li>Совместимость с различными панелями</li>
                  <li>Долговечность конструкции</li>
                </ul>
              </div>

              <div className="flex flex-col sm:flex-row gap-4">
                <Button size="lg" className="px-8">
                  Заказать консультацию
                </Button>
                <Button variant="outline" size="lg" className="px-8">
                  Скачать каталог
                </Button>
              </div>
            </div>
          </div>
        </div>
      </main>
    </div>
  );
}